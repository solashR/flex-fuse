...
def task_project_build(project, output_dir='flex_fuse_resources', tag='igz',
    ...
    tasks_to_run = [
        ...
        },
        {
            'name': 'save_images',
            'args': {
                'output_filepath': os.path.join(save_images_dir, 'flex-fuse-docker-images'),
...
def task_save_images(project, images, output_filepath=None):
    ...
    try:
        ...
        yield ziggy.shell.run(project.ctx, 'docker stop {}'.format(registry_temp_name))
